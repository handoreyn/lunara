...
public sealed class SendMessageServiceTests
{
        ...
        FakeBlockChecker? blockChecker = null)
    {
        ...
        SendMessageService svc = new(matchSvc, convRepo, msgRepo, uow, clock, outbox, checker);
        return new Fixtures(svc, matchSvc, convRepo, msgRepo, uow, outbox, checker);
    }
    ...
    /// <summary>Sending a message when the sender blocked the recipient throws <see cref="InvalidOperationException"/>.</summary>
    [Fact]
    public async Task SendAsync_WhenSenderBlockedRecipient_ThrowsInvalidOperationException()
    {
        FakeBlockChecker blockChecker = new();
        blockChecker.SeedBlock(SenderA.Value, SenderB.Value);
        Fixtures f = Build(blockChecker: blockChecker);

        await Assert.ThrowsAsync<InvalidOperationException>(
            () => f.Svc.SendAsync(DefaultRequest(sender: SenderA), CancellationToken.None));
    }

    /// <summary>Sending a message when the recipient blocked the sender throws <see cref="InvalidOperationException"/>.</summary>
    [Fact]
    public async Task SendAsync_WhenRecipientBlockedSender_ThrowsInvalidOperationException()
    {
        FakeBlockChecker blockChecker = new();
        blockChecker.SeedBlock(SenderB.Value, SenderA.Value);
        Fixtures f = Build(blockChecker: blockChecker);
    ...
    [Fact]
    public async Task SendAsync_WhenBlockExists_NoMessageOrConversationPersisted()
    {
        FakeBlockChecker blockChecker = new();
        blockChecker.SeedBlock(SenderA.Value, SenderB.Value);
        Fixtures f = Build(blockChecker: blockChecker);
